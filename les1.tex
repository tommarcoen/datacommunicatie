...
\item
   ...
   \item Nokia heeft SR~OS of TiMOS.
   \end{itemize}
\item
...
\item
   ...
   Deze kan je downloaden van \href{https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html}{https://www.chiark.greenend.org.uk/~sgtatham/putty/}.
   Ofwel download je de Windows installer (MSI), ofwel download je \texttt{putty.exe}.
   \begin{description}
   ...
   \end{description}
\item
   De prompt toont de hostname van het toestel en een groter-dan-teken (\emph{user exec mode});
   gebruik het commando \texttt{enable} om over te schakelen naar de modus waarin je alle rechten hebt (\emph{privileged exec mode}).
\item
...
\item het subnet mask van de computer,
\item de default gateway.
\end{enumerate}
Het subnet mask bepaalt welk gedeelde van het IP-adres aangeeft in welk netwerk het zich bevindt (de straatnaam) en welk gedeelde de computer uniek identificeert (het huisnummer).

\paragraph{Bijvoorbeeld:}
\begin{itemize}
\item Het subnet mask is 255.255.255.0; dit betekent dat de eerste drie nummers (want drie keer 255) de straatnaam of het netwerk aanduiden en het laatste blokje (de 0 in het subnet masker) het huisnummer aangeeft.
...
\end{itemize}
Het subnet mask moet niet altijd 255.255.255.0 zijn maar kan bijvoorbeeld ook 255.255.0.0 zijn. In dit geval geven de twee eerste nummers van het IP-adres de straatnaam weer en de laatste twee nummers het huisnummer (het is dus een heel groot netwerk of een heel lange straat met veel huisnummers).
...
\paragraph{Bijvoorbeeld:}
\begin{itemize}
\item Het subnet mask is 255.255.255.0.
